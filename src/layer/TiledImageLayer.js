...
TiledImageLayer.prototype.cleanCache = function (dc) {

    var now = Date.now();

    if (now - this.lastClean < 6 * 60 * 1000) {
        return;
    }

    /*console.log('clean tile cache begin');
     console.log('gpuResourceCache',Object.keys(dc.gpuResourceCache.entries.entries).length);
     console.log('tileCache',Object.keys(this.tileCache.entries).length);*/

    var tileCacheEntries = this.tileCache.entries;

    var tileToRemove = [];
    var resourcesToRemove = [];
    var i;

    for (var key in tileCacheEntries) {
        if (tileCacheEntries.hasOwnProperty(key)) {
            if (now - tileCacheEntries[key].lastUsed < 5 * 60 * 1000) {
                tileToRemove.push(key);
                for (i = 0; i < tileCacheEntries[key].entry.length; i++) {
                    resourcesToRemove.push(tileCacheEntries[key].entry[i].gpuCacheKey);
                }
            }
        }
    }

    for (i = 0; i < tileToRemove.length; i++) {
        this.tileCache.removeEntry(tileToRemove[i]);
    }

    for (i = 0; i < resourcesToRemove.length; i++) {
        dc.gpuResourceCache.removeResource(resourcesToRemove[i]);
    }

    this.lastClean = now;

    /*console.log('clean tile cache done');
     console.log('gpuResourceCache',Object.keys(dc.gpuResourceCache.entries.entries).length);
     console.log('tileCache',Object.keys(this.tileCache.entries).length);*/
