...
SurfaceTileRendererProgram.prototype.loadTexSamplerMatrix = function (gl, matrix) {
    ...
    var forceLoad = false;
    if (this.localState.texSamplerMatrix === null) {
        forceLoad = true;
        this.localState.texSamplerMatrix = Matrix.fromIdentity();
    }

    if (forceLoad || !this.localState.texSamplerMatrix.equals(matrix)) {
        this.loadUniformMatrix(gl, matrix, this.texSamplerMatrixLocation);
        this.localState.texSamplerMatrix.copy(matrix);
    }
};
...
SurfaceTileRendererProgram.prototype.loadTexMaskMatrix = function (gl, matrix) {
    ...
    var forceLoad = false;
    if (this.localState.texMaskMatrix === null) {
        forceLoad = true;
        this.localState.texMaskMatrix = Matrix.fromIdentity();
    }

    if (forceLoad || !this.localState.texMaskMatrix.equals(matrix)) {
        this.loadUniformMatrix(gl, matrix, this.texMaskMatrixLocation);
        this.localState.texMaskMatrix.copy(matrix);
    }
};
...
SurfaceTileRendererProgram.prototype.loadTexSampler = function (gl, unit) {
    if (this.localState.textureUnit !== unit) {
        gl.uniform1i(this.texSamplerLocation, unit - gl.TEXTURE0);
...
SurfaceTileRendererProgram.prototype.loadOpacity = function (gl, opacity) {
    if (this.localState.opacity !== opacity) {
        gl.uniform1f(this.opacityLocation, opacity);
        this.localState.opacity = opacity
    }
};
...
SurfaceTileRendererProgram.prototype.loadModulateColor = function (gl, enable) {
    if (this.localState.modulateColor !== enable) {
        gl.uniform1i(this.modulateColorLocation, enable ? 1 : 0);
        this.localState.modulateColor = enable;
    }
};
