...
BasicTextureProgram.prototype.loadTextureEnabled = function (gl, enable) {
    if (this.localState.textureEnabled !== enable) {
        gl.uniform1i(this.textureEnabledLocation, enable ? 1 : 0);
        this.localState.textureEnabled = enable;
    }
};
...
BasicTextureProgram.prototype.loadTextureMatrix = function (gl, matrix) {
    var forceLoad = false;
    if (this.localState.textureMatrix === null) {
        forceLoad = true;
        this.localState.textureMatrix = Matrix.fromIdentity();
    }

    if (forceLoad || !this.localState.textureMatrix.equals(matrix)) {
        this.loadUniformMatrix(gl, matrix, this.textureMatrixLocation);
        this.localState.textureMatrix.copy(matrix);
    }
};
...
BasicTextureProgram.prototype.loadApplyLighting = function (gl, applyLighting) {
    if (this.localState.applyLighting !== applyLighting) {
        gl.uniform1i(this.applyLightingLocation, applyLighting);
        this.localState.applyLighting = applyLighting;
    }
};
