...
import NavigatorState from '../navigate/NavigatorState';
...
import TextSupport from '../render/TextSupport';
...
DrawContext.unitQuadKeyInterleaved = "DrawContextUnitQuadKeyInterleaved";
...
DrawContext.prototype.unitQuadBufferInterleaved = function () {
    var vboId = this.gpuResourceCache.resourceForKey(DrawContext.unitQuadKeyInterleaved);

    if (!vboId) {
        var gl = this.currentGlContext;
        var vertexData = new Float32Array(20);

        vertexData[0] = 0; // upper left corner
        vertexData[1] = 1;
        vertexData[2] = 0;
        vertexData[3] = 0; // upper left corner
        vertexData[4] = 1;

        vertexData[5] = 0; // lower left corner
        vertexData[6] = 0;
        vertexData[7] = 0;
        vertexData[8] = 0; // lower left corner
        vertexData[9] = 0;

        vertexData[10] = 1; // upper right corner
        vertexData[11] = 1;
        vertexData[12] = 0;
        vertexData[13] = 1; // upper right corner
        vertexData[14] = 1;

        vertexData[15] = 1; // lower right corner
        vertexData[16] = 0;
        vertexData[17] = 0;
        vertexData[18] = 1; // lower right corner
        vertexData[19] = 0;
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, null);
        this.gpuResourceCache.putResource(this.unitQuadKeyInterleaved, vboId, 80);
    }
